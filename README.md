...
- 홈: 이번 달 평균 순위 / 1위 횟수 / 라스 횟수 / 누적 우마 (지난달 대비), 이번 달 랭킹(MVP), 최근 대국
- 기록: 월별 필터, 상세 보기, 삭제
- 랭킹: 월별 / 전체 기간, 누적 우마 기준
- 멤버: 추가·수정, 대국 기록이 있는 멤버는 비활성 처리
...
src/
  ...
  components/             # 공용 UI (버튼, 카드, 아바타, 순위 배지, 하단 내비 등)
  pages/                  # 홈 / 대국 기록 / 기록 목록·상세 / 랭킹 / 멤버
...
| 멤버 아바타 | `public/images/avatars/<memberId>.png` |

- 초기 샘플 멤버의 ID 는 `yeonkyung`(연경), `minsu`(민수), `jisu`(지수), `hyunwoo`(현우) 입니다.
- `public/images/avatars/avatar_5.png` ~ `avatar_10.png` 는 새 멤버용 예비 아바타입니다. 멤버 수정 화면의 **아바타 파일명** 에 `avatar_5.png` 처럼 넣으면 바로 쓸 수 있습니다.
- 새로 추가한 멤버의 ID 와 정확한 아바타 경로는 멤버 화면 → 연필(수정) 버튼에서 확인할 수 있습니다. 멤버의 **아바타 파일명** 을 지정하면 그 파일명을 대신 사용합니다.
- 이미지가 없거나 로드에 실패하면 아바타 영역은 숨겨지고 이름만 표시되며, 마스코트가 없으면 여백만 남습니다.
- 홈 마스코트는 투명 배경 PNG 에 가로가 넉넉한 비율(예: 600×440), 아바타는 정사각형(예: 240×240)을 권장합니다.
...
홈 화면의 "내 통계" 는 헤더 오른쪽(또는 "나 선택") 에서 고른 멤버를 기준으로 하며 이 기기(localStorage)에만 기억됩니다.
